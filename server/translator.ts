...
const translationInstructions = [
  ...
  '每個 region 都要用 0 至 1000 正規化座標提供 bubble 同 safe：bubble 精準框住一個對話泡／旁白框嘅可見範圍；safe 完整包住泡內所有原文字，稍微外擴，但不可碰到泡框線。',
  'safe 必須完全位於 bubble 內，唔可以包含泡框線、人物、分鏡線、相鄰泡框或其他文字。泡框即使被頁面邊緣截斷、開放或只顯示一部分，只要文字清楚可讀，就一定要回傳；bubble 貼齊可見頁面邊界即可。只可以忽略無法判斷內容或位置嘅文字。',
